              ...
              <div className="row-2">
                {
                  cardNumberArray.map( (ele,index) =>{
                    return <span className={index%4 === 0 ? "space" : ""}>{cardNumber[index] ? cardNumber[index]: "#"}</span>
                  })
                }
              </div>
              <div className="row-3">
                <div className="card-holder">
                  ...
                  <span style={{textTransform: "uppercase"}}>{name === "" ? "FULL NAME" : name}</span>
                </div>
        ...
        <div className="form">
          ...
          >
            ...
            <Form.Item
              ...
              colon={false}
            >
              <Input onChange={e => setName(e.target.value)} onMouseEnter={e=>setFlip(false)}/>
            </Form.Item>
            <Form.Item
              colon={false}
              label="Expiration Date"
            >
              <DatePicker picker="month" onChange={e => setExpirationMonth(moment(e).format('M'))} allowClear={false} onMouseEnter={e=>setFlip(false)}/>
              <DatePicker picker="year" onChange={e => setExpirationYear(moment(e).format('YY'))} allowClear={false} onMouseEnter={e=>setFlip(false)}/>
            </Form.Item>
